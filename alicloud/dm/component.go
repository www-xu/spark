...
func (c *Component) Instantiate() (err error) {
	err = c.ctx.UnmarshalKey("alicloud_dm", &c.config)
	if err != nil {
		return err
	}

	if c.config == nil {
		return errors.New("alicloud_dm config isn't found")
	}
	...
	c.instance, err = dm20151123.NewClient(&openapi.Config{
...
func Get(ctx context.Context) *dm20151123.Client {
	return instance.Get(ctx)
}

func (c *Component) Get(ctx context.Context) *dm20151123.Client {
	return c.instance
}
